...
function WeatherTable({ handleRequest, cityName }) {
  ...
  let content = (
    <div className={styles.updateContainer}>
      <p>Update data please</p>
      <button type="button" onClick={() => handleRequest(cityName, data.currentApi)}>
        {' '}
      </button>
    </div>
  );
  ...
  if (data.currentApi === 'openWeatherApi') {
    if (data.currentCity !== null && data.openWeatherApi !== null) {
      if (data.currentCity.city.toLowerCase() in data.openWeatherApi) {
        content = data.openWeatherApi[data.currentCity.city.toLowerCase()].list.map(
          (elem, index) => {
            if (index === 0) {
              firstElement.push(
                <WeatherTodayItem
                  key={nanoid()}
                  degree={elem.main.temp}
                  description={elem.weather[0].main}
                  weatherId={elem.weather[0].id}
                />,
              );
            } else {
              return (
          ...
          if (index === 0) {
            firstElement.push(
              <WeatherTodayItem
                key={nanoid()}
                degree={elem.temp}
                description={getWeatherDescription(elem.weathercode)}
              />,
            );
          } else {
            return (
