...
function CitySelector({handleRequest}) {
  ...
  return (
    <select className={styles.select} value={optionValue} onChange={(e) => {
      ...
      Object.keys(weatherData[weatherData.currentApi]).map((elem) => <option key={nanoid()} value={elem}>{`${elem[0].toUpperCase()}${elem.slice(1)}`}</option>)  
    : null}
    </select>
  )
}
