...
const weatherReducer = (state = defaultState, action) => {
  switch (action.type) {
    ...
    case ADD_CITY_OPEN_METEO_API:
      return {...state, openMeteo: {...state.openMeteo, [action.payload.city.city.toLowerCase()]: action.payload}};
    case SET_WEATHER_DATA_OPEN_METEO_API:
      return {...state, openMeteo: action.payload}
...
export const addCityOpenMeteoApi = (payload) => ({type: ADD_CITY_OPEN_METEO_API, payload});
export const setCurrentApiActionCreator = (payload) => ({type: SET_CURRENT_API, payload})
export const setWeatherDataOpenMeteoActionCreator = (payload) => ({type: SET_WEATHER_DATA_OPEN_METEO_API, payload});
export const setLoadingValueActionCreator = (payload) => ({type: SET_LOADING_VALUE, payload});
